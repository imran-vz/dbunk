  ...
  runQuery: async (tabId, options) => {
    ...
    try {
      ...
      set((state) => ({
        ...
        // Inlined `applyConnectionUpdate` until the runQuery action moves
        // into the relational-queries slice (commit 8) — at which point
        // this should call `get().applyConnectionActivity(tab.connectionId, nowIso)`
        // or similar via a Connections-slice cleanup method.
